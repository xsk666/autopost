...
def surplus(head, email):
    ...
    pattern = pattern[22:26] + pattern[-2:]
    return pattern
...
def login(uuid, email, password):
    ...
    try:
        ...
        ip = test[13]
        expire = test[17]
        head = {"cookie": "__cfduid="+uuid+"; lang=zh-cn; cnxad_lunbo=yes; _ga=GA1.2.1656003110.1594518881; _gid=GA1.2.1056328585.1594518881; uid="+uid+"; email="+email+"; key="+key+"; ip="+ip+"; expire_in="+expire}
...
def onSign(email, password):
    try:
        response = requests.get("https://paoluz.link/auth/login")
        cookie = response.headers["set-cookie"]
        cfuid = cookie.split(";")[0]
